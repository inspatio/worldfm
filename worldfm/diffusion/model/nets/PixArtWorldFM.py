...
#   https://github.com/PixArt-alpha/PixArt-sigma/blob/master/diffusion/model/nets/PixArt.py
# --------------------------------------------------------
import math
import torch
...
from worldfm.diffusion.model.nets.PixArtWorldFM_blocks import t2i_modulate, CaptionEmbedder, AttentionKVCompress, MultiHeadCrossAttention, T2IFinalLayer, TimestepEmbedder, LabelEmbedder, FinalLayer
...
class PixArtWorldFMBlock(nn.Module):
    """
    A PixArtWorldFM block with adaptive layer norm (adaLN-single) conditioning.
    """
...
#############################################################################
#                            Core PixArtWorldFM Model                              #
#################################################################################
@MODELS.register_module()
class PixArtWorldFM(nn.Module):
    """
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, **kwargs):
            ...
            try:
                ...
                if os.environ.get("PROPE_CACHE_DEBUG", "0") == "1":
                    ...
                    if rank0:
                        ...
                        logger.info("[PRoPE][cache][error] PixArtWorldFM.forward failed to prepare shared prope_cache; will fall back to per-layer compute.\n" + traceback.format_exc())
        else:
            kwargs["prope_cache"] = None
        ...
        if use_plucker:
            ...
            plucker = plucker.permute(0, 2, 1, 3, 4).contiguous().view(B, h_tokens * w_tokens_total, 6)
        ...
        for i, block in enumerate(self.blocks):
            x = auto_grad_checkpoint(block, x, y, t0, y_lens, (self.h, self.w), block_id=i, **kwargs)  # (N, T, D) #support grad checkpoint
    ...
    def forward_with_cfg(self, x, timestep, y, cfg_scale, mask=None, **kwargs):
        """
        Forward pass of PixArtWorldFM, but also batches the unconditional forward pass for classifier-free guidance.
        """
        # https://github.com/openai/glide-text2im/blob/main/notebooks/text2im.ipynb
        half = x[: len(x) // 2]
    ...
    def initialize_weights(self):
        ...
        # Zero-out adaLN modulation layers in PixArtWorldFM blocks:
...
#################################################################################
#                              PixArtWorldFM Configs                                #
#################################################################################
@MODELS.register_module()
def PixArtWorldFM_XL_2(**kwargs):
    return PixArtWorldFM(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)
