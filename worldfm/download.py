...
"""
Functions for downloading pre-trained PixArtWorldFM models
"""
...
def download_model(model_name):
    ...
    local_path = f'weights/{model_name}'
    if not os.path.isfile(local_path):
        hf_endpoint = os.environ.get("HF_ENDPOINT")
        if hf_endpoint is None:
            hf_endpoint = "https://huggingface.co"
        os.makedirs('weights', exist_ok=True)
        web_path = f'{hf_endpoint}/inspatio/worldfm/tree/main/{model_name}' #TODO: change to WorldFM hf repository
        download_url(web_path, 'weights/')
